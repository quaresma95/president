...
class President extends Table {
    ...
    protected function setupNewGame($players, $options = []) {    
        ...
        switch ($rule_set) {
            case 0:
                ...
                self::setGameStateInitialValue("unskippable_23", 0);
        ...
        if (!self::getGameStateValue('same_rank_skip')) self::setGameStateValue("unskippable_23", 0);
    ...
    */
    ...
    protected function getAllDatas() {
        ...
        $result['permanent_pass'] = self::getGameStateValue("permanent_pass");
...
////////////    
    ...
    */
    ...
    function getSurvivingNextPlayer ($player_id) {
        $permanent_pass = self::getGameStateValue("permanent_pass");
        ...
        while ((!in_array($next_player, $remaining_players) || ($permanent_pass && $this->getUniqueValueFromDB("SELECT player_has_passed FROM player WHERE player_id = $next_player") > 0)) && $next_player != $player_id)
    ...
    */
    ...
    function playCard ($card_ids) {
        ...
        if ($table_combination) {
            ...
            else if ($hand_combination['value'] == $table_combination['value']) {
                if (!self::getGameStateValue("same_rank_skip")) throw new BgaUserException(self::_("Same rank skip is not allowed in this table"));
                else if (self::getGameStateValue("unskippable_23") && ($hand_combination['value'] == 13 || $hand_combination['value'] == -1)) throw new BgaUserException(self::_("2 (or 3 during card rank reversal) cannot be skipped in this table"));
            }
        ...
        $permanent_pass = self::getGameStateValue("permanent_pass");
        if (!$permanent_pass) $this->DbQuery("UPDATE player SET player_has_passed = 0");
        ...
        if ($table_combination) {
            if ($hand_combination['value'] == $table_combination['value'] && !($ender_8 && $count_8) && $hand_player_count > 1) {
                ...
                if (!$permanent_pass) $this->DbQuery("UPDATE player SET player_has_passed = 1 WHERE player_id = $next_player");
                self::notifyAllPlayers($permanent_pass ? 'noSound' : "passTurn", clienttranslate('Playing the same rank skips ${player_name}\'s turn!'), [
    ...
    */
        ...
        if ($hand_player_count <= 1) $transition = 'endRound';
            ...
            if ($ender_8 && $found_8) $transition = 'endTrick';
                ...
                if ($table_combination) {
                    ...
                    $unskippable_23 = self::getGameStateValue("unskippable_23");
                    $locked_count = [];
                    $count_joker = 0;
                    $count_8 = 0;
                    $count_9 = 0;
                    ...
                    if ($playable) {
                        ...
                        if ($hand_combination['value'] < $table_combination['value']) $playable = false;
                        if ($hand_combination['value'] == $table_combination['value'])
                            if (!$same_rank_skip) $playable = false;
                            else if ($unskippable_23 && ($hand_combination['value'] == 13 || $hand_combination['value'] == -1)) $playable = false;
                        ...
                        if ($playable) {
                            ...
                            if ($revolution && count($hand) >= 4 && $hand_player_count > 1) {
                                self::setGameStateValue("regular_revolution", 1 - $regular_revolution);
                                self::incStat(1, 'revolution', $next_player);
                                self::notifyAllPlayers("noSound", clienttranslate('Playing 4 or more cards reverses the card rank during this round!'), []);
                            }
                            if ($jack_back && $count_jack && !($ender_8 && $count_8) && $hand_player_count > 1) {
                                self::setGameStateValue("temporary_revolution", 1 - $temporary_revolution);
                            ...
                            if ($table_combination) {
                                if ($hand_combination['value'] == $table_combination['value'] && !($ender_8 && $count_8) && $hand_player_count > 1) {
                                    ...
                                    if (!$permanent_pass) $this->DbQuery("UPDATE player SET player_has_passed = 1 WHERE player_id = $next_next_player");
                                    self::notifyAllPlayers($permanent_pass ? 'noSound' : "passTurn", clienttranslate('Playing the same rank skips ${player_name}\'s turn!'), [
                            ...
                            if ($lock && $hand_player_count > 1) {
                                self::incStat(1, 'suit_lock', $next_player);
                                self::notifyAllPlayers("noSound", clienttranslate('Playing the same suit combination locks the suit during this trick!'), []);
                            }
                    ...
                    if (!$autoplay) {
                        $skippable = $same_rank_skip && !($unskippable_23 && ($table_combination['value'] == 13 || $table_combination['value'] == -1));
                        if (count($hand) < count($table_cards) || $table_combination['value'] == 14 || (!self::getUniqueValueFromDB("SELECT count(*) FROM card WHERE card_type = 4") && $unskippable_23 && ($table_combination['value'] == 13 || $table_combination['value'] == -1)) || (count($table_cards) == 1 && count($hand) == 1)) $autopass = true;
                        else if (self::getGameStateValue("automatic_skip")) {
                            $current_revolution_status = (bool)self::getGameStateValue("regular_revolution") ^ (bool)self::getGameStateValue("temporary_revolution");
                            $multiplier = $current_revolution_status ? -1 : 1;
                            $hand_cards = array_merge(($hand_player_count < 3 && !$this->cards->countCardInLocation('deck') && !$this->cards->countCardInLocation('removed') ? $this->cards->getCardsInLocation('hand', $next_player) : $this->cards->getCardsInLocation('hand')), $this->cards->getCardsInLocation('deck'), $this->cards->getCardsInLocation('removed'));
                            ...
                            switch ($table_combination['type']) {
                                default:
                                    ...
                                    if ($autopass) foreach ($value_color_array as $value => $colors) {
                                        ...
                                        if (count($colors) + $remaining_joker_count >= count($table_cards) && ($true_value > $table_combination['value'] || ($skippable && $true_value == $table_combination['value']))) {
                                            $autopass = false;
                                            break;
                                        }
                                    }
                                ...
                                case 1:
                                    ...
                                    foreach ($color_value_array as $values) {
                                        $current_check_value = $table_combination['value'] * $multiplier + ($skippable ? 0 : $multiplier);
                                        if ($current_revolution_status) {
    ...
    function stEndTrick() {
        ...
        if (count($hand_player_count) <= 1) $transition = 'endRound';
            ...
            $zombie_count = 0;
            foreach ($hand_player_count as $hand_player => $count) if ($this->isPlayerZombie($hand_player)) $zombie_count++;
            if ($zombie_count >= count($hand_player_count)) {
                $transition = 'endRound';
                $playable = false;
                $zombies = array_keys($hand_player_count);
                shuffle($zombies);
                foreach ($zombies as $key => $zombie_id) $this->DbQuery("UPDATE player SET player_role = ".(101 + $key)." WHERE player_id = $zombie_id");
            }
